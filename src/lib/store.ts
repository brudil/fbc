...
import { ParticipantData, Participant, MessageData, Message, ThreadDataFile } from './dataTypes';
...
async function buildSchema() {
    ...
    await db.schema.createTable(Table.ThreadParticipants, (table) => {
        table.increments();
        table.string('name');
        table.integer('friend').unsigned().nullable();
...
function notUndefined(x: InspectTreeResult | undefined): x is InspectTreeResult {
    return x !== undefined;
}
...
export async function loadData(path: string, progressor: Progressor): Promise<Data> {
    await buildSchema();
    ...
    const profile: any = await readAsync(
        join(path, 
            findFile('profile_information.json', findDir('profile_information', tree)).relativePath
        ),
    'json');
    ...
    const messageThreads = await Promise.all(allMessageFiles.map(loadMessageThread));

    progressor.loadedThreads();

    const totalMessages = messageThreads.reduce((prev: number, thread: ThreadDataFile) => prev + thread.messages.length, 0)
    progressor.setMessages(totalMessages);

    await Promise.all(messageThreads.map(async (thread) => {
        ...
        const participantsIds = await Promise.all(thread.participants.map(async (participant: ParticipantData) => {
        ...
        // @ts-ignore
        const participants = await db(Table.ThreadParticipants).whereIn('id', participantsIds);
        ...
        await Promise.all(thread.messages.map(async (message: MessageData) => {
            let p = participantMap.hasOwnProperty(message.sender_name) ? participantMap[message.sender_name].id : null;
            ...
            if (p === null) {
                ...
                if (isUnknownUser) {
                    if (unknownUser === null) {
                        unknownUser = await addLeftParticipant('Unknown user', threadRow[0]);
                    }

                    p = (unknownUser as Participant).id;
                } else {
                    const part: Participant = await addLeftParticipant(message.sender_name, threadRow[0]);
                    p = part.id;
        ...
        console.log(`inserted ${insertMessages.length} messages in to ${thread.title}`);
