...
import { totalSentMessages, percentOfMessagesSelf, biggestChats, selfMessagesByMonth, selfTrendByMonth } from '../../../lib/analysers/analysers';
...
export const Overview = (props: OverviewProps) => {
    ...
    return (
        <StandardView>
            <StatsGrid>
                <StatBox value={totalSent} subline="messages sent" formatter={bigNumber} />
                <StatBox value={percentageSelf} subline="chatterboxing" formatter={(v) => `${v}%`} />
                <StatBox value={chatsBySize} subline="largest chat" formatter={v => v[0].title} fitText />
            </StatsGrid>

            {(xoxo.data === null || babe.data === null) ? null : <Line
                data={{
            ...
            {selfMessages.data === null ? null : <Line
                data={{
                    ...
                    datasets: [
                      {
                        ...
                        pointRadius: 1,
                        pointHitRadius: 10,
                        data: selfMessages.data.reduce((a, m) => {a.push(m.total + a[a.length - 1]); return a}, [0]).slice(1)
                      }
                    ]
                  }}
            />}
